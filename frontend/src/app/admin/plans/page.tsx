...
export default function PlansPage() {
  ...
  const [form, setForm] = useState({ name: "", price: 0, ads_count: 1, featured_count: 0, note: "" });

  const [editingId, setEditingId] = useState<string | null>(null);
  const [editForm, setEditForm] = useState({ name: "", price: 0, ads_count: 1, featured_count: 0, note: "", is_active: true });
  ...
  async function handleCreate() {
    ...
    setSaving(true);
    try { await createPlan(form); toast.success("Creado"); setForm({ name: "", price: 0, ads_count: 1, featured_count: 0, note: "" }); load(); }
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6 mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            ...
            <input type="number" value={form.price || ""} onChange={e => setForm({...form, price: Number(e.target.value)})} placeholder="Precio S/"
              className="rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-200" />
            <input type="number" value={form.ads_count || ""} onChange={e => setForm({...form, ads_count: Number(e.target.value)})} placeholder="N° anuncios"
              className="rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-200" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <input type="number" value={form.featured_count || ""} onChange={e => setForm({...form, featured_count: Number(e.target.value)})} placeholder="N° destacados"
              className="rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-200" />
            <input value={form.note} onChange={e => setForm({...form, note: e.target.value})} placeholder="Nota (opcional)"
              className="rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-200" />
          </div>
                  ...
                  {editingId === p.id ? (
                    <div className="flex-1 grid grid-cols-2 sm:grid-cols-3 gap-2">
                      <input value={editForm.name} onChange={e => setEditForm({...editForm, name: e.target.value})}
                        className="rounded border border-gray-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-200" />
                      <input type="number" value={editForm.price || ""} onChange={e => setEditForm({...editForm, price: Number(e.target.value)})}
                        className="rounded border border-gray-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-200" />
                      <input type="number" value={editForm.ads_count || ""} onChange={e => setEditForm({...editForm, ads_count: Number(e.target.value)})}
                        className="rounded border border-gray-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-200" />
                      <input type="number" value={editForm.featured_count || ""} onChange={e => setEditForm({...editForm, featured_count: Number(e.target.value)})}
                        className="rounded border border-gray-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-200" />
                    ...
                    <>
                      <div className="flex-1 grid grid-cols-2 sm:grid-cols-4 gap-2">
                        <div>
                          ...
                          <p className="text-xs text-gray-400">S/ {p.price}</p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-700">{p.ads_count} anuncio{p.ads_count !== 1 ? "s" : ""}</p>
                          {p.featured_count > 0 && <p className="text-xs text-gray-400">+{p.featured_count} destacado{p.featured_count !== 1 ? "s" : ""}</p>}
                        </div>
                      ...
                      <button onClick={() => { setEditingId(p.id); setEditForm({ name: p.name, price: p.price, ads_count: p.ads_count, featured_count: p.featured_count, note: p.note || "", is_active: p.is_active }); }}
