...
export class DatabaseService implements OnModuleInit {
  ...
  private async ensureColumns() {
    const migrations = [
      ...
      `ALTER TABLE buyer_requests ADD COLUMN IF NOT EXISTS cantidad_objetivo INT`,
      `ALTER TABLE buyer_requests ADD COLUMN IF NOT EXISTS cmc INT DEFAULT 1`,
      `ALTER TABLE buyer_requests ADD COLUMN IF NOT EXISTS precio_objetivo NUMERIC(10,2)`,
      `ALTER TABLE buyer_requests ADD COLUMN IF NOT EXISTS ua VARCHAR(30)`,
      `ALTER TABLE buyer_requests ADD COLUMN IF NOT EXISTS ficha_tecnica JSONB`,
      `ALTER TABLE buyer_requests ADD COLUMN IF NOT EXISTS nivel_coincidencia VARCHAR(20) DEFAULT 'estricta'`,
      `ALTER TABLE buyer_requests ADD COLUMN IF NOT EXISTS visibilidad_ofertas VARCHAR(20) DEFAULT 'publica'`,
      `ALTER TABLE request_offers ADD COLUMN IF NOT EXISTS stock INT`,
      `ALTER TABLE request_offers ADD COLUMN IF NOT EXISTS plazo_entrega_dias INT`,
      `ALTER TABLE request_offers ADD COLUMN IF NOT EXISTS beneficios TEXT`,
      ...
      `INSERT INTO app_config (key, value) VALUES ('garantia_subasta_inversa_pct', '5'), ('garantia_demanda_agregada_pct', '5'), ('demanda_agregada_pct', '70') ON CONFLICT (key) DO NOTHING`,
