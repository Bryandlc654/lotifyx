...
async function main() {
  ...
  for (const row of r.rows) {
    console.log(`Orden: ${row.id.slice(0,8)}...`);
    console.log(`  N° Operación: ${row.operation_number}`);
    console.log(`  Total: S/ ${row.total_amount}`);
    console.log(`  Estado: ${row.status}`);
    console.log(`  Producto: ${row.items?.[0]?.title || 'SIN PRODUCTO'}`);
    console.log(`  Puja: ${row.bid ? `S/ ${row.bid.amount}` : 'N/A'}`);
    console.log(`  OK: ${row.items?.[0]?.title ? '✅' : '❌'}`);
    console.log('');
  }
  await client.end();
}
